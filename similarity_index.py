...
def predictExpCFRating(userId,itemId,userRatings,foldNo,actualRating,simThreshold,confThreshold):
	...
	if len(userRatings) == 0 : # cold-start	
		...
		return avgRating,avgRating # for cold start, return avg item rating and full coverage	
	meanRatingUser		= 	sum(userRatings.values()) / len(userRatings)
	...
	if noExpertsRatedItem < confThreshold: # confidence threshold ,no prediction made, return user mean and full coverage
		...
		return avgRating,meanRatingUser
	...
	return avgRating,predictedRating
...
def testExpertCF(expertSet,noCVFolds,simThreshold,confThreshold,minExpertRating,noTestUsers,kNN):	
	#file	= open(path + "results_"+str(minExpertRating)+"/results_summary_"+str(minExpertRating)+"_sim_"+str(simThreshold)+"_conf_"+str(confThreshold)+".csv", 'wb')
	#writer 	= csv.writer(file)
	...
	for foldNo in range(1,noCVFolds+1):
		print "Started testExpertCF() for minExpertRating = ",minExpertRating," and for Fold no = ", foldNo, "and simThreshold = ", simThreshold,"and confThreshold = ",confThreshold, " and no of test users = ", noTestUsers
		#file2 	= open(path + "results_"+str(minExpertRating)+"/results_fold_no_" + str(foldNo) +".csv", 'wb')
		#writer2 	= csv.writer(file2)
		#writer2.writerow(["foldNo","simThreshold","confThreshold","userName","userId","itemName","itemId","newUser","expertCoverage","expertModCoverage","actualRating", "avgExpertRating" ,"predictedExpertRating","predictedExpertRatingMod"])
		noUsersTested	=	0
		yStdPred			=	[]
		yAvgExpPred			=	[]
		yTrue				=	[]
		...
		testSet			=	valSetList[foldNo-1].values()
		#print "userId, itemId, simThreshold, confThreshold, actualRating, predictedStandardCFRating, predictedExpertRating, predictedExpertModRating"			
		for testUserDetails in testSet:
			newUser=1
			...
			if len(userRatings):# user seen before in the system
					...
					predictExpCFRating(userId,itemId,userRatings,foldNo,actualRating,simThreshold,confThreshold)
			#predictedStandardCFRating,stdCoverage= predictStandardCFRating(userId,itemId,userRatings,foldNo,kNN)
			'''
			print userId, itemId, simThreshold, "	", confThreshold,"	",actualRating, "	", predictedExpertRating,"	",predictedExpertModRating
			#writer2.writerow([foldNo,simThreshold,confThreshold,userName.replace('\"',''),userId.replace('\"',''),itemName.replace('\"',''),itemId.replace('\"',''),newUser,expertCoverage,expertModCoverage,actualRating,avgExpertRating,predictedExpertRating,predictedExpertModRating])						
			yTrue.append(actualRating)
			yStdPred.append(predictStandardCFRating)
		...
		#writer.writerow([foldNo,minExpertRating,simThreshold,confThreshold,maeAvgExpCF,rMSEAvgExp,maeExpCF,rMSEExp,meanExpCoverage,maeExpModCF,rMSEExpMod,meanExpModCoverage])
		'''
def testExpCFSim(foldNo,minRating,simValues,confThreshold,noTestUsers):
	file	= open(path + "results_vary_sim_fold_no_"+str(foldNo)+"_rating_"+str(minRating)+"_users_"+str(noTestUsers)+".csv", 'wb')
	writer 	= csv.writer(file)
	writer.writerow(["foldNo","sim","noTestUsers","maeExpCF","rMSEExp"])
	#print "foldNo,sim,noTestUsers,maeExpCF,rMSEExp"	
	testSet			=	valSetList[foldNo-1].values()
	trainSet		=	trainSetList[foldNo-1]
	for simThreshold in simValues:
		yTrue				=	[]
		yExpPred			=	[]
		stdCoverageList		=	[]
		maeExpCF			=	0.0
		rMSEExp				=	0.0				
		predictedExpRating	=	0.0
		avgRating			=	0.0
		noUsersTested		=	0
		for testUserDetails in testSet:	
			if noUsersTested > noTestUsers:
			...
			itemId 		 = testUserDetails[3]
			actualRating = testUserDetails[4]
			userRatings=getUserRatingsFromDict(userId,foldNo)			 
			if len(userRatings):
				noUsersTested	=noUsersTested 	+	1
				populateExpertSimMatrix(expertSet,userId,userRatings,foldNo)
				avgRating,predictedExpRating	=	predictExpCFRating(userId,itemId,userRatings,foldNo,actualRating,simThreshold,confThreshold)
				yTrue.append(actualRating)
				yExpPred.append(predictedExpRating)
		maeExpCF		=	calcMAE(yTrue, yExpPred)
		rMSEExp			=   calcRMSE(yTrue, yExpPred)
		print foldNo,simThreshold,noTestUsers,maeExpCF,rMSEExp	
		writer.writerow([foldNo,simThreshold,noTestUsers,maeExpCF,rMSEExp])
...
noCVFolds=1
noTestUsers=10
simThreshold=0#0.01#0.4
simThresholdValues=[]
for i in range(0,2000):
	simThreshold=simThreshold+0.0005
	simThresholdValues.append(round(simThreshold,6))
confThreshold=0#5
kNN=30
minRatingValues=[100,500]#[100,250,500,750]
...
for minRating in minRatingValues:
	...
	#testExpertCF(expertSet,noCVFolds,simThreshold,confThreshold,minRating,noTestUsers,kNN)
	testExpCFSim(1,minRating,simThresholdValues,confThreshold,noTestUsers)

...
def testExpertCF(expertSet,noCVFolds,simThreshold,confThreshold,minExpertRating,noTestUsers,kNN):	
	...
	for foldNo in range(1,noCVFolds+1):
		...
		#writer.writerow([foldNo,minExpertRating,simThreshold,confThreshold,maeAvgExpCF,rMSEAvgExp,maeExpCF,rMSEExp,meanExpCoverage,maeExpModCF,rMSEExpMod,meanExpModCoverage])
		'''
def testExpCFConf(foldNo,minRating,simThreshold,confThresholdValues,noTestUsers):
	file	= open(path + "results_vary_conf_fold_no_"+str(foldNo)+"_rating_"+str(minRating)+"_users_"+str(noTestUsers)+".csv", 'wb')
	writer 	= csv.writer(file)
	writer.writerow(["foldNo","confThreshold","noTestUsers","maeExpCF","rMSEExp"])
...
#confThreshold=0#5
confThresholdValues=[i for i in range(0,61,1)] #[1,10,20]
...
for minRating in minRatingValues:
	...
	testExpCFConf(1,minRating,simThreshold,confThresholdValues,noTestUsers)
